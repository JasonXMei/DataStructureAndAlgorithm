...
public class TwoSum1 {
    ...
    public static void main(String[] args) {
        int[] arr = {2, 11, 7, 15};
        int target = 9;
        int[] index = method1(arr, target);
        for(int i : index){
            System.out.println(i);
        }

        index = method2(arr, target);
        for(int i : index){
            System.out.println(i);
        }

        index = method3(arr, target);
        for(int i : index){
            System.out.println(i);
        }
    }
    ...
    public static int[] method2(int[] arr, int target){
        ...
        for(int i=0;i<arr.length;i++){
            int expectedNum = target - arr[i];
            if(tempMap.containsKey(expectedNum) && tempMap.get(expectedNum) != i){
                return new int[]{i, tempMap.get(expectedNum)};
            }
        }
        return new int[]{};
    }
